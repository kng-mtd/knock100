...
### hierarchy level
```{sql}
create table if not exists table0 (id,child,parent);
```
```{sql}
...
with table1 as(
  ...
  union all
  select t1.id,t1.child,t1.parent,t2.lv+1
  from table0 as t1
  inner join table1 as t2 on t1.parent=t2.name
)
select * from table1;
```
...
```{sql}
create table if not exists table0 (node1,node2);
```
```{sql}
...
with table1 as(
  ...
  union all
  select t1.node1,t1.node2,t2.depth+1
  from table0 as t1
  inner join table1 as t2 on t1.node1=t2.node2
  where t2.depth<10 --max search depth
)
select node2 as goal,min(depth) as depth from table1
