...
```{sql}
create virtual table vrt1 using fts5 (txt1,txt2,tokenize='ascii'); -- ascii keep apostropy
```
```{sql}
...
### Bag of Word
```{sql}
create virtual table vrt1 using fts5 (txt,tokenize='ascii'); -- ascii keep apostropy
```
```{sql}
...
, tmp2 as(
    select term,doc,count(1) as n from vrt1_inst
    where term glob '[a-z]*' and term not glob '*[0-9]*'
    group by term,doc
)
select doc, tmp1.term as term, n
from tmp1 inner join tmp2 on tmp1.term=tmp2.term 
order by doc, term;
```
...
sum of all docs: na
```{sql}
select count(distinct doc) as na from vrt1_inst;
```
...
tf-idf
```{sql}
with doc_word_count as (
    select doc, sum(n) as total_words
    from wiki_bow
    group by doc
),
term_doc_freq as (
    select term, count(distinct doc) as doc_freq
    from wiki_bow
    group by term
),
tfidf as (
    select 
        b.term, 
        b.doc, 
        (b.n * log((select count(distinct doc) from wiki_bow) / (1.0 + tdf.doc_freq))) / dwc.total_words as tfidf_score
    from wiki_bow b
    join term_doc_freq tdf on b.term = tdf.term
    join doc_word_count dwc on b.doc = dwc.doc
)
select * from tfidf;
```
