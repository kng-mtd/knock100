...
```{r}
dbExecute(con,
          "create temp table tmp01 (chr1 text, num1 int, num2 int, num3 int)")
sql0="insert into tmp01 (chr1, num1, num2, num3) values "
...
dbExecute(con,sql)
sql=paste0(sql0,"('b',2,null,400)")
dbExecute(con,sql)
sql=paste0(sql0,"('c',3,40,500)")
dbExecute(con,sql)
```
```{sql}
select * from tmp01;
...
select store_cd, n,sum, min, max, med, mean
  , sqrt((sq-mean*mean)*n/(n-1)) as sd
  , sqrt((sq-mean*mean)*n/(n-1))/mean as cv
from tmp1;
```
...
select store_cd, n, sum(amount) as sum
  ...
  , max(amount-0) as max
  , avg(amount) as mean
  , sqrt((avg(sq)-avg(amount)*avg(amount))*n/(n-1)) as sd
  , sqrt((avg(sq)-avg(amount)*avg(amount))*n/(n-1))/avg(amount) as cv
from tmp2
group by store_cd;
```
...
## mode
```{sql}
select amount, count(1) as n from receipt
group by amount
having count(1)>=(select max(n)
                  from(select count(1) as n from receipt
                  group by amount));
```
...
create temp table tmp0 as
  with tmp1 as(
    select count(1) as n0, avg(age) as mean0
      , avg(age*age) as sq0 from customer
  )
  , tmp2 as(
    select count(1) as na, avg(age) as mean_a
      , avg(age*age) as sq_a from customer
    where gender_cd='0'
  )
  , tmp3 as(
    select count(1) as nb, avg(age) as mean_b
      , avg(age*age) as sq_b from customer
    where gender_cd='1' 
  )
  select n0, mean0
  , sqrt((sq0-mean0*mean0)*n0/(n0-1)) as sd0
  , na, mean_a
  , sqrt((sq_a-mean_a*mean_a)*na/(na-1)) as sd_a
  , nb, mean_b
  , sqrt((sq_b-mean_b*mean_b)*nb/(nb-1)) as sd_b
  from tmp1, tmp2, tmp3;
```
