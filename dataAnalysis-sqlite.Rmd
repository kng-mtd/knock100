...
with tmp1 as (
  select 2 as n
  union all
  select n+1 from tmp1 where n<1000
),
tmp2 as (
...
select n from tmp2 limit 10;
```

### prime number using rowid instead of recursive sql
```{sql}
drop table if exists dum;
```
```{sql}
create table if not exists dum (dum int);
```
```{sql}
insert into dum (dum)
	select null from (select 0 union all select 0 union all select 0 union all select 0)
	cross join (select 0 union all select 0 union all select 0 union all select 0)
	cross join (select 0 union all select 0 union all select 0 union all select 0)
	cross join (select 0 union all select 0 union all select 0 union all select 0)
	cross join (select 0 union all select 0 union all select 0 union all select 0)
	cross join (select 0 union all select 0 union all select 0 union all select 0)
...
```
```{sql}
select min(rowid), max(rowid) from dum;
```
