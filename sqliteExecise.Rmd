...
## 3 rows have highest value by group
```{sql}
...
create table score as
	with tmp1 as(
		select 0 as id, null as grp, null as val
		union all
		select id+1 as id, abs(floor(random()/1e18)) as grp, floor(random()/1e15) as val
		from tmp1
		where id<100
	)
	select * from tmp1 where id!=0
;
```
...
select grp, id, val, rank from tmp1 where rank between 1 and 3
;
```
