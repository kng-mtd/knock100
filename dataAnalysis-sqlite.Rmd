...
## process to null
```{sql}
create temp table tmp1 (col1 int, col2 int, col3 int, col4 text);
```
```{sql}
insert into tmp1 (col1 ,col2, col3, col4) values
  (1,10,10,'aaa')
  ,(2,20,20,null)
  ,(3,null,'','ccc')
  ,(4,40,40,'');
...
```{sql}
select count(1), count(col1), count(col2), count(col3), count(col4)
  ,sum(col1), sum(col2), sum(col3), sum(col4)
  ,avg(col1), avg(col2), avg(col3), avg(col4) from tmp1;
```
...
```{sql}
select grp, count(1), avg(age) from tmp1 group by grp, gender_cd;
```
