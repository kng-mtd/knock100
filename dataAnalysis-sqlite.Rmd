...
### rows have null
answer of alithmetric includes null is null,  
text type column's value is 0 when its used alithmetric
```{sql}
select * from tmp01
...
### fill null with back value
```{sql}
create temp table dum as
with tmp1 as(
  select 0 as ts, random()%100 as val
  union all
  select ts+1, iif(random()%4=0, null, random()%100) from tmp1
  where ts<20
)
select * from tmp1;
```
...
```{sql}
with tmp1 as(
        select row_number() over(order by ts) as i,* from dum
)
, tmp2 as(
        select i, ts, val from tmp1 where i=1
        union all
        select tmp1.i as i, tmp1.ts as ts
        , coalesce(tmp1.val, tmp2.val)
        from tmp1 join tmp2
        on tmp1.i=tmp2.i+1
)
select * from tmp2 order by i;
```

### functions for null
```
coalesce(col1, col2..., val)

nullif(col1, val)
ex.
select avg(nullif(col1, 'NA')) from table
```
...
```{sql}
with tmp0 as(
	select 120 as a, 216 as b --a<b
)
, tmp1 as(
	select 0 as i, * from tmp0
	union all
	select i+1 as i, b%a as a, a as b from tmp1
	where a!=0
)
--select * from tmp1;
select tmp0.a as a, tmp0.b as b
, tmp1.b as gcd
, tmp0.a*tmp0.b/tmp1.b as lcm
from tmp1, tmp0
where tmp1.a=0;
```
...
-- x**2-10=0
with tmp1 as (
  select 0 as l, 10 as r, (0+10)/2.0 as x
  union all
  select
    case when power(x,2)-10<0 then x else l end as l,
    case when power(x,2)-10<0 then r else x end as r,
    case when power(x,2)-10<0 then (x+r)/2.0 else (l+x)/2.0 end as x
  from tmp1
  where r-l>1e-6
)
select * from tmp1;
...
### iterative method
```{sql}
-- x1=g(x0), x1->x0 ,when  |g'(x0)|<1
-- x**2-x-1=0 -> x-1-1/x= 0 -> x=1+1/x
with tmp1 as(
	select 0 as i, 1.0 as x
	union all
	select i+1 as i
	, 1.0+1.0/x as x
	from tmp1 	where  abs(x-1.0-1.0/x)>1e-6
)
select * from tmp1
;
```
...
-- x**2-10=0
with tmp1 as(
	select 0 as i, 10.0 as x
	union all
	select i+1 as i
	, x-(power(x,2)-10.0)/((power(x+0.001,2)-10.0)-(power(x,2)-10.0))*0.001 as x
	from tmp1	where abs(power(x,2)-10.0)>1e-6
)
select * from tmp1
;
```
...
select node2 as goal, depth, visited from(
  select *, row_number() over (partition by node2 order by depth) as n    from tmp1
)
where n=1;
```
...
select node2 as goal, depth, weight as distance, path
from (select *
  , row_number() over (partition by node2 order by weight) as n from tmp1)
where n=1
;
```
