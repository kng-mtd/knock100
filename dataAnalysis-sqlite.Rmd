...
#### prime factorization using recursive twice
```{sql}
with tmp1(n, p) as(
  select 11111111, 2 -- n, 2 minimum prime

  union all
  select n/p, p
  from tmp1 where n%p=0 --step1 keep dividing n by p 

  union all
  select n, (select p from prime where p>tmp1.p limit 1) --step2 get next minimum prime 
  from tmp1 where n%p<>0 and n>1
)
