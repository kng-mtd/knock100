...
```{sql}
create virtual table vrt1 using fts5 (txt1,txt2);
```
```{sql}
...
### count term
count term and document include it in all column and row
```{sql}
...
count term and document include it by column in all row
```{sql}
...
count term and document include it by column and row
```{sql}
