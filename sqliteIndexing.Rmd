...
```
----covering index
--index column is (caluculated) to output column

create index idx_receipt on receipt (sales_ymd, amonut);

select sales_ymd, sum(amount) from receipt group by sales_ymd;

drop index idx_receipt;


----partial index
--indexing to part of rows, removing needless rows (null, dummy,...), it decrese address table size

create index idx_receipt on receipt (customer_id);

select * from receipt where customer_id='CS022615000144';

drop index idx_receipt;

create index idx_receipt on receipt (customer_id) where customer_id='ZZ000000000000';
...
create index idx_receipt on receipt (lower(customer_id)); 

select * from receipt where lower(customer_id)='CS022615000144';
